...
Use a clean teaching workspace with no client files or real credentials on screen. Before Week 2, rehearse NotebookLM MCP end-to-end: install (`codex mcp add notebooklm npx notebooklm-mcp@latest`), auth, open a **demo notebook with 20+ mixed sources**, ask one cited question. Keep UI-add-from-list as fallback. **No client secrets, piracy, or paywall hacking.**
...
From Week 2, add: “Skills are reusable instructions. MCP is a live tool plug. Today the plug is NotebookLM — gather 15–25 sources, keep cited insights.”
...
| 2 | NotebookLM research power session via MCP (15–25 sources, ≥4 types) |
...
- Week 2 uses NotebookLM + public research sources. Later weeks may reuse that MCP or another approved plug. Treat real email/calendar logins as Week 6 only, read-only, with explicit consent and least permissions.
...
For MCP demos: show the config entry (or Settings → MCP servers), restart/reload if needed, then ask Codex to call a NotebookLM tool and show counts/citations. Insist learners open `sources/source-log.md` and insight files themselves.
...
| Student pastes client secrets / pirated PDFs in W2 | Stop; public reputable sources only. |
...
| One learner finishes early | Observer role or Week 2 stretch (missing type only, ≤25). |
| MCP will not connect | Check path, command, restart Codex; use UI fallback from source-log. |
